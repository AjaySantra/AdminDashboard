...
function Add() {
    ...
    if (BrandName.trim() == "") {
        showMessage('info', 'Info', 'Please enter brand name.');
        $("#brandname").focus();
        $.LoadingOverlay("hide");
        return;
    }

    var jsonString = '{"BrandName": "' + BrandName
        + '","CreatedBy":"' + CreatedBy + '"}';
...
function GetData() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.ResponseCode == 200) {
                if (data.Data.Brand.length > 0) {
                    // $('#lblTotal').html(data.d.length);
                    console.log(data);
                    bindGetAllSRList(data.Data.Brand);
                    $.LoadingOverlay("hide");
                    return false;
                }
                else {
...
function ClearTable() {
    $('#gvDetailsList').empty();
    // $("#btnrowList").empty();
    $("#gvDetailsList").html("<thead><tr><th></th><th></th><th></th><<th>Sl.No</th><th>Brand Name</th><th>Created By</th><th>Created Date</th></tr></thead>");
}
...
function bindwithpagingList(psizeList, data, from, to) {
    ClearTable();
     for (var i = from; i < to; i++) {
        $("#gvDetailsList").append("<tr><td><a href='#myModal' role='button' data-toggle='modal' onclick='ShowSRDetails(\"" + data[i].BrandId + "\")'>View</a></td><td><a href='#' >Edit</a></td><td><a  href='#' onclick='deleteItem()'>Delete</a></td><td>" + (i+1) + "</td><td>" + data[i].BrandName + "</td><td>" + data[i].CreatedBy + "</td> <td>" + data[i].CreatedDate + "</td></tr>");
    }
}