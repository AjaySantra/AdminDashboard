...
function Add() {
    ...
    if (FrameColor.trim() == "") {
        showMessage('info', 'Info', 'Please enter frame color.');
        $("#framecolor").focus();
        $.LoadingOverlay("hide");
        return;
    }

    var jsonString = '{"FrameColor": "' + FrameColor
        + '","CreatedBy":"' + CreatedBy + '"}';

    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: BaseURL + "framecolor/add",
        data: jsonString,
        dataType: "Json",
...
function GetData() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url : BaseURL + "framecolor/get",
        data : jsonString,
        dataType : "Json",
        success: function (data) {
            if (data.ResponseCode == 200) {
                if (data.Data.FrameColor.length > 0) {
                    // $('#lblTotal').html(data.d.length);
                    console.log(data);
                    bindGetAllSRList(data.Data.FrameColor);
                    $.LoadingOverlay("hide");
                    return false;
                }
                else {
...
function ClearTable() {
    $('#gvDetailsList').empty();
    // $("#btnrowList").empty();
    $("#gvDetailsList").html("<thead><tr><th></th><th></th><th></th><<th>Sl.No</th><th>Frame Color</th><th>Created By</th><th>Created Date</th></tr></thead>");
}
...
function bindwithpagingList(psizeList, data, from, to) {
    ClearTable();
     for (var i = from; i < to; i++) {
        $("#gvDetailsList").append("<tr><td><a href='#myModal' role='button' data-toggle='modal' onclick='ShowSRDetails(\"" + data[i].ColorId + "\")'>View</a></td><td><a href='#' >Edit</a></td><td><a  href='#' onclick='deleteItem()'>Delete</a></td><td>" + (i+1) + "</td><td>" + data[i].FrameColorName + "</td><td>" + data[i].CreatedBy + "</td> <td>" + data[i].CreatedDate + "</td></tr>");
    }
}