...
function Add() {
    ...
    if (ProductType.trim() == "") {
        showMessage('info', 'Info', 'Please enter product type.');
        $("#producttype").focus();
        $.LoadingOverlay("hide");
        return;
    }

    var jsonString = '{"ProductType": "' + ProductType
        + '","CreatedBy":"' + CreatedBy + '"}';

    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: BaseURL + "producttype/add",
        data: jsonString,
        dataType: "Json",
...
function GetData() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url : BaseURL + "producttype/get",
        data : jsonString,
        dataType : "Json",
        success: function (data) {
            if (data.ResponseCode == 200) {
                if (data.Data.ProductType.length > 0) {
                    // $('#lblTotal').html(data.d.length);
                    console.log(data);
                    bindGetAllSRList(data.Data.ProductType);
                    $.LoadingOverlay("hide");
                    return false;
                }
                else {
...
function ClearTable() {
    $('#gvDetailsList').empty();
    // $("#btnrowList").empty();
    $("#gvDetailsList").html("<thead><tr><th></th><th></th><th></th><<th>Sl.No</th><th>Product Type</th><th>Created By</th><th>Created Date</th></tr></thead>");
}
...
function bindwithpagingList(psizeList, data, from, to) {
    ClearTable();
     for (var i = from; i < to; i++) {
        $("#gvDetailsList").append("<tr><td><a href='#myModal' role='button' data-toggle='modal' onclick='ShowSRDetails(\"" + data[i].TypeId + "\")'>View</a></td><td><a href='#' >Edit</a></td><td><a  href='#' onclick='deleteItem()'>Delete</a></td><td>" + (i+1) + "</td><td>" + data[i].ProductType + "</td><td>" + data[i].CreatedBy + "</td> <td>" + data[i].CreatedDate + "</td></tr>");
    }
}